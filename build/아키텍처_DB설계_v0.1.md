...
- **AI 자동 분류**: 워커가 문서 타입(산업/기업/뉴스)과 산업(표준 24 세트)을 분류해 `reports.doc_type`/`industry_id` 에 기록(사용자 확인·수정). 취업 렌즈는 사용자 직무(`user_lenses.config.jobRole`)에 따라 추출 관점을 조정.
...
UNIQUE (user_id, slug)
```
> 글로벌 카탈로그 = 네이버 증권 업종을 계열 통합한 표준 **24개 세트**(반도체·AI·IT·소프트웨어·디스플레이·전기전자·게임·미디어·광고·통신·자동차·로봇·조선·기계·운송·물류·철강·금속·석유·화학·에너지·유틸리티·환경·기후·건설·섬유·의류·화장품·음식료·제약·바이오·유통·여행·교육·금융·기타). 업로드 문서의 산업은 워커가 이 세트 중 하나로 AI 매칭.
