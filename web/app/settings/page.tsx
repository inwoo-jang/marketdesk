...
export default function SettingsPage() {
  ...
  // 로컬 에이전트(Ollama)
  const [agent, setAgent] = useState<{ enabled: boolean; email: string | null } | null>(null);
  const [agentBusy, setAgentBusy] = useState(false);

  async function toggleAgent(enabled: boolean) {
    setAgentBusy(true);
    const r = await api.setLocalAgent(enabled).catch(() => null);
    setAgentBusy(false);
    if (r) setAgent((a) => ({ enabled: r.enabled, email: a?.email ?? null }));
  }
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-10">
      ...
      <section className="mt-4 rounded-card bg-card p-6 shadow-card">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-semibold text-ink-muted">로컬 에이전트 · Ollama</h2>
          <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">헤비 유저</span>
        </div>
        <p className="mt-1 text-xs text-ink-muted">
          본인 PC의 <b className="text-ink">Ollama</b>(로컬 오픈모델)로 분석해요. 완전 무료·무제한, 데이터도 내 컴퓨터 밖으로 안 나가요. 켜면 새 업로드는 로컬 에이전트가 처리합니다.
        </p>
        <div className="mt-3 flex items-center gap-2">
          {agent?.enabled ? (
            <>
              <span className="rounded-lg bg-success-bg/40 px-3 py-1.5 text-sm font-medium text-success-text">사용 중 · Ollama</span>
            ...
            <button onClick={() => toggleAgent(true)} disabled={agentBusy} className="rounded-lg border border-line px-3 py-1.5 text-sm font-medium hover:bg-bg-deep disabled:opacity-50">Ollama로 켜기</button>
          )}
        </div>
        {agent?.enabled && (
          <div className="mt-3 rounded-lg border border-line bg-bg-deep/40 p-3">
            ...
            <p className="mt-1 text-[11px] text-ink-muted">① ollama.com 설치 → ② 모델 받기 → ③ 본인 작업만 로컬에서 처리(아래).</p>
            <pre className="mt-2 overflow-x-auto rounded bg-ink/5 p-2 text-[11px] text-ink">{`ollama pull llama3.1

LOCAL_AGENT_USER_EMAIL=${agent.email ?? "you@example.com"} \\
LLM_PROVIDER=ollama \\
