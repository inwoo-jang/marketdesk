...
- **읽기 보조**: 핵심 하이라이트 강조 + 단어 풀이(본문 단어 클릭/검색 → AI 100자 설명, ~1초, 맥락 반영, 약어 정식명 병기) + 형광펜(5색) + **메모**(본문 선택 → 우측 여백 노트, 스크롤 동행) + 영문 기사는 한국어로 요약.
- **정리·탐색**: 리포트·공공 콘텐츠 **책갈피·숨김** + 탭(전체/즐겨찾기/숨긴 항목) + **페이지네이션**. 기업리포트는 **계열별/산업별 필터**(공정위 계열사 매핑 = SK→SK하이닉스·11번가 등 전 계열사), 관련 **뉴스도 리스트에 함께**.
- **흐름 보드**: 월/년 × 산업별(★)·기업별(계열)·경제흐름 가로 타임라인. 셀(요약 키워드) → **흐름 피드(요약+근거 원문)** → 원문 드릴다운. 업로드 시 자동 갱신.
